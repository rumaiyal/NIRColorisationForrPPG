...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    ...
    for label, im_data in visuals.items():
        im = util.tensor2im9bit(im_data)
        image_name = '%s_%s.tif' % (name, label)
        save_path = os.path.join(image_dir, image_name)
        #util.save_image(im, save_path, aspect_ratio=aspect_ratio)
        #images saved to 16 bit depth  
        util.save_image_9bittiff(im, save_path)
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            for label, image in visuals.items():
                ...
                ###Pls note no matter how many bit depth the original image, visuals are only saved to 8 bit depth
